...
//         // Reset the counter value to the start value
//         setCount(0);
//
//         // Reset the max value to its initial value (5 in this case)
//         setMaxValue(5);
//
//         // Reset the start value to its initial value (0 in this case)
//         setStartValue(0);
//
//         // Hide the settings section after resetting
//         // setShowSettings(false);
//     }
//
//     const errorValue = startValue >= maxValue || startValue < 0;
//     const countMaxValue = count === maxValue
//
//     const inputError = errorValue ? 'error' : undefined;
//
//
//     return (
...
//                     <button onClick={handleIncrement} disabled={count === maxValue || errorValue}>
//                         Increment
//                     </button>
//                     <button disabled={errorValue} onClick={handleReset}>Reset</button>
//                 </div>
//
//             </div>
...
import React, {useState} from 'react';
import './counter.css';
import reset from './../img/reset.svg'; // Импорт изображения для кнопки сброса

const Counter = () => {
    const [count, setCount] = useState(0); // Состояние для хранения текущего значения счетчика
    const [maxValue, setMaxValue] = useState(5); // Состояние для хранения максимального значения счетчика
    const [startValue, setStartValue] = useState(0); // Состояние для хранения начального значения счетчика
            ...
            {/* Раздел с настройками */}
            {
                showSettings && (
                    <div className={'counter-settings'}>
                        ...
                        <div className={'counter-input'}>
                            {/* Здесь могут быть другие настройки, если нужно */}
                        </div>
                        {/* Кнопка для установки настроек */}
                        <button onClick={handleSet} disabled={startValue >= maxValue || startValue < 0}>
                            Set
                        </button>
                    </div>
